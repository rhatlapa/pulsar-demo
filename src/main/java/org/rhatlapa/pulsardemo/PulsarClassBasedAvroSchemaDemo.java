...
public class PulsarClassBasedAvroSchemaDemo {
	...
	public static void main(String[] args) throws IOException {
		try (var pulsarClient = PulsarClient.builder()
			...
			try (var testDataProducer = pulsarClient.newProducer(Schema.AVRO(TestData.class)).topic(TOPIC_WITH_AVRO).create()) {
			...
			try (var reader = pulsarClient.newReader(Schema.AVRO(TestData.class)).topic(TOPIC_WITH_AVRO)
